...
import {
  ...
  Wallet, Gift, Star, Coffee, Scissors, Utensils, SlidersHorizontal, ChevronRight, Store, Hotel, Coins, TrendingUp, X, Sparkles
} from "lucide-react";
...
export default function HistoryPage() {
  ...
  return (
    React.createElement('div', { className: "space-y-6" }
      ...
      /* Extra Points coming soon banner */
      , React.createElement('div', { className: "bg-gradient-to-br from-indigo-50 to-purple-50 border border-purple-100 rounded-3xl p-4 flex items-center gap-3.5 shadow-sm" }
          , React.createElement('div', { className: "w-10 h-10 rounded-2xl bg-purple-600 text-white flex items-center justify-center shrink-0" }
            , React.createElement(Sparkles, { className: "h-5 w-5" })
          )
          , React.createElement('div', null
            , React.createElement('p', { className: "text-xs font-black text-slate-800" }, "Exchange Extra Points!")
            , React.createElement('p', { className: "text-[10px] text-slate-600 font-semibold mt-0.5" }, "You can collect and exchange extra points with expensive rewards. This feature is coming soon!")
          )
        )
                        ...
                        , React.createElement('div', { className: "flex-1 bg-white rounded-3xl border border-slate-100 p-4 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex items-center justify-between gap-3 hover:scale-[1.01] active:scale-[0.99] transition-all ml-4" }
                            ...
                            , React.createElement('div', { className: "flex items-center gap-2 shrink-0 text-right" }
                              , React.createElement('div', { className: "space-y-1" }
                                ...
                                , React.createElement('p', { className: "font-black text-xs text-[#F97316]" }, `+${item.points ?? 0} Points`)
                              )
                ...
                React.createElement(DialogHeader, { className: "flex flex-col items-center" },
                 ...
                 React.createElement("div", { className: "w-full bg-slate-50 rounded-2xl p-4 border border-border/50 text-left space-y-2.5 text-xs" },
                  React.createElement("div", { className: "flex justify-between" },
                    React.createElement("span", { className: "text-muted-foreground font-semibold" }, "Purchasing Amount"),
                    React.createElement("span", { className: "font-bold text-foreground" }, selectedItem.data.spendAmount ? `₹${Number(selectedItem.data.spendAmount)}` : "—")
                  ),
                  React.createElement("div", { className: "flex justify-between" },
                    React.createElement("span", { className: "text-[#F97316] font-semibold" }, "Total Points"),
                    React.createElement("span", { className: "font-bold text-[#F97316]" }, `+${selectedItem.data.points ?? 0} pts`)
                  ),
                  React.createElement("div", { className: "flex justify-between" },
                    React.createElement("span", { className: "text-purple-600 font-semibold" }, "Extra Points"),
                    React.createElement("span", { className: "font-bold text-purple-600" }, `+${selectedItem.data.extraPoints ?? 0} pts`)
                  ),
                  React.createElement("div", { className: "flex justify-between" },
